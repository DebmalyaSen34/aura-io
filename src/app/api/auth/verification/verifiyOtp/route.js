...
export async function POST(request) {
    try {
        ...
        if (!storedOtp) {
            ...
            return NextResponse.json({ success: false, message: 'No otp was found with you email address!' }, { status: 404 });
        }

        if (storedOtp !== otp) {
            console.log('Invalid otp was provided!');
            return NextResponse.json({ success: false, message: 'Invalid otp was provided!' }, { status: 401 });
        }
        ...
        if (!userDataJson) {
            ...
            return NextResponse.json({ success: false, message: 'User data was not found!' }, { status: 404 });
        }
        ...
        if (userError) {
            console.log('There was an error while registering you into out database!');
            return NextResponse.json({ success: false, message: 'Error while inserting into database!' }, { status: 500 });
        }
